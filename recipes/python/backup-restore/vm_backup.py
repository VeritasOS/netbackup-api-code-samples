...
# Perform VM backup
def perform_vm_backup(baseurl, token, protection_plan_id, asset_id, is_vm_group=0):
...
def get_backup_job_id(baseurl, token, backup_job_id, protection_plan_name):
    """ This function will return the catalog and protection plan backup id"""
    print("Find the protection plan and catalog backup job IDs")
    protection_backup_job_id = ''
    catalog_backup_job_id = ''
    headers.update({'Authorization': token})
    url = f"{baseurl}admin/jobs/?filter=jobId gt {str(backup_job_id)}"
    ...
    for jobdata in response_text['data']:
        ...
        if jobdata['attributes']['policyName'].startswith('NBU_Catalog_Default') \
           and job_id == parent_job_id:
