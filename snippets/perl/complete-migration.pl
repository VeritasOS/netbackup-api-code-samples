...
sub printUsage {
  print "\nUsage : perl complete_migration.pl -nbmaster <master_server> -login_username <login_username> -login_password <login_password> [-login_domainname <login_domain_name> -login_domaintype <domain_type>] [-reason <reason>] [--force] [--verbose]\n\n";
...
if (defined $reason) {
    migration::complete_migration($base_url, $token, $reason, $force);
} elsif ($force) {
    migration::complete_migration($base_url, $token, $force);
} else {
    migration::complete_migration($base_url, $token);
}
