...
use FindBin;
sub printUsage {
  print "\nUsage : perl get_asset_groups.pl -nbmaster <master_server> -username <username> -password <password> [-domainname <domain_name>] [-domaintype <domain_type>]\n\n";
  die;
}
...
my $jsonString = asset_group::get_asset_groups($master_server, $token);
print "$jsonString\n";
