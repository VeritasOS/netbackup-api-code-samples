...
sub printUsage {
  print "\nUsage : perl patch_asset_groups.pl -nbmaster <master_server> -username <username> -password <password> [-domainname <domain_name>] [-domaintype <domain_type>] -guid <assetGroupGuid>\n\n";
  die;
}
...
my $jsonString = asset_group::patch_asset_groups($master_server, $token, $guid);
print "$jsonString\n";
