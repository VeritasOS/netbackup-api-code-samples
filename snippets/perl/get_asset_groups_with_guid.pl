...
sub printUsage {
  print "\nUsage : perl get_asset_groups_with_guid.pl -nbmaster <master_server> -username <username> -password <password> [-domainname <domain_name>] [-domaintype <domain_type>] -guid <guid>\n\n";
  die;
}
...
my $jsonString = asset_group::get_asset_groups_with_guid($master_server, $token, $guid);
print "$jsonString\n";
