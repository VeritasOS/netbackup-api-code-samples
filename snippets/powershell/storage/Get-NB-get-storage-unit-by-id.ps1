...
.SYNOPSIS
This sample script demonstrates the use of NetBackup REST API for fetching scpecified storage unit.

.DESCRIPTION
This script will fetch scpecified storage unit.

.EXAMPLE
./Get-NB-get-storage-unit-by-id.ps1 -nbmaster "nb-master.example.com" -username "administrator" -password "secret" -stu_name "storage unit name"
#>
...
$sts_uri = $basepath + "/storage/storage-units/" + $stu_name
...
{
    throw "Unable to fetch storage unit." 
}

Write-Host "storage unit fetched successfully.`n"
