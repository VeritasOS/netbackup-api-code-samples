...
.SYNOPSIS
This sample script demonstrates the use of NetBackup REST API for fetching the replication targets on a disk volume

.DESCRIPTION
This script will fetch scpecified disk pool.

.EXAMPLE
./Get-NB-get-disk-pool-by-id.ps1 -nbmaster "nb-master.example.com" -username "administrator" -password "secret" -dpid "disk pool id" -reptargetid "replication targetid"
#>
...
param (
    ...
    [string]$dvid        = $(throw "Please specify the storage server  using -dvid parameter.")
    [string]$reptargetid = $(throw "Please specify the replication target id using -reptargetid parameter.")
)
...
Write-Host "Replication targets fetched successfully.`n"
