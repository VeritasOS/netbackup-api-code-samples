...
.SYNOPSIS
This sample script demonstrates the use of NetBackup REST API for fetching scpecified storage server.

.DESCRIPTION
This script will fetch scpecified storage server.

.EXAMPLE
./Get-NB-get-storage-server-by-id.ps1 -nbmaster "nb-master.example.com" -username "administrator" -password "secret" -stsid "storage server id"
#>
...
{
    throw "Unable to fetch storage server." 
}

Write-Host "storage server fetched successfully.`n"
