...
.SYNOPSIS
This sample script demonstrates the use of NetBackup REST API for deletion of replication target added at disk volume level for msdp

.DESCRIPTION
This script will delete replication target added at disk volume level for msdp

.EXAMPLE
./Post-NB-delete-replication-target-to-disk-volume.ps1 -nbmaster "nb-master.example.com" -username "administrator" -password "secret" -stsid "storage server id" -dvid "diskvolume id"
#>
...
#####################################################################
# POST Delete replication target on disk volume level with specified details.
#####################################################################
...
$response_delete_replication_target_on_dv = Invoke-WebRequest             `
                    -Uri $sts_uri                `
                    -Method POST                 `
                    -Body ($json)        `
...
if ($response_delete_replication_target_on_dv.StatusCode -ne 204)
{
    throw "Unable to delete replication target." 
}

Write-Host "Replication target on with the specified details on disk volume is deleted successfully.`n"
echo $response_delete_replication_target_on_dv
Write-Host $response_delete_replication_target_on_dv

$response_delete_replication_target_on_dv = (ConvertFrom-Json -InputObject $response_create_sts)
